...
class ManageAbbreviations(Base):
    ...
    def verifyDataFrameUpdatedOrNot(self, count1, count2):
        if count2 > count1:
            self.LogScreenshot.fLogScreenshot(message=f'Before updating Abbreviation and Definition in template number '
                                                      f'of rows in file {count1}, after updating Abbreviation and '
                                                      f'Definition in file - number of rows in file {count2}',
