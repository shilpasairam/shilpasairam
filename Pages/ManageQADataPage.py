        ...
        try:
            for i in study_data:
                ...
                if actual_upload_status_text == expected_upload_status_text:
                    ...
                    self.LogScreenshot.fLogScreenshot(message=f'Error in file uploading',
                ...
                if qafile.equals(excelfile):
                    ...
                    raise Exception("File contents between QA File '{Path(f'{i[1]}').stem}' and Complete Excel Report '{Path(f'ActualOutputs//{excel_filename1}').stem}' are not matching")

        except:
            raise Exception("Unable to upload the Manage QA Data")

    def del_data_after_qafile_comparison(self, study_data, del_locator, del_locator_popup, pop_value):
        expected_delete_status_text = 'QA excel file successfully deleted'
        self.go_to_manageqadata("manage_qa_data_button")
