...
class Test_UtilityOutcome_QOL:
    ...
    @pytest.mark.C26937
    def test_qol_presenceof_utilitysummary_into_excelreport(self, extra, env):
        ...
        for i in self.pop_list:
            try:
                ...
                for j in self.slrtype:
                    ...
                    self.utiloutcome.qol_presenceof_utilitysummary_into_excelreport(excel_filename, util_filepath)

            except Exception:
                raise Exception("Unable to select element")

    @pytest.mark.C26956
    def test_qol_verify_source_to_target_row_counts_in_excelreport(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
    ...
    @pytest.mark.C27567
    def test_qol_presenceof_utilitysummary_into_wordreport(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
        ...
        for i in self.pop_list:
            try:
                ...
                for j in self.slrtype:
                    ...
                    self.utiloutcome.qol_presenceof_utilitysummary_into_wordreport(word_filename, util_filepath)

            except Exception:
                raise Exception("Unable to select element")
    
    @pytest.mark.C27569
    def test_qol_verify_source_to_target_row_counts_in_wordreport(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)         
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
    ...
    @pytest.mark.C26938
    def test_qol_verify_excelreport_utility_summary_sorting_order(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
    ...
    @pytest.mark.C27568
    def test_qol_verify_wordreport_utility_table_sorting_order(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
    ...
    @pytest.mark.C30281
    def test_qol_utility_outcome_results_comparison_newimport_logic(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
    ...
    @pytest.mark.C30280
    @pytest.mark.C31399
    def test_qol_utility_outcome_results_comparison_oldimport_logic(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
        ...
        for i in self.pop_list:
            try:
                ...
                for j in self.slrtype:
                    ...
                    # word_filename1 = self.slrreport.getFilenameAndValidate(180)
                    word_filename = self.slrreport.get_latest_filename(UnivWaitFor=180)
                    self.utiloutcome.validate_filename(word_filename, util_filepath, 'OldImportLogic')
    ...
    @pytest.mark.C31498
    def test_qol_validate_utilitysummarytab_and_contents_into_excelreport(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Creating object of loginpage class
        self.loginPage = LoginPage(self.driver, extra)
        ...
        for index, i in enumerate(scenarios):
            try:
                self.utiloutcome.qol_validate_utilitysummarytab_and_contents_into_excelreport(i, util_filepath,
                                                                                              index, env)
            except Exception:
                raise Exception("Unable to select element")

    @pytest.mark.C26906
    def test_qol_presenceof_newlyadded_utilitycolumn_names(self, extra, env):
        baseURL = ReadConfig.getApplicationURL(env)
        util_filepath = ReadConfig.getutilityoutcome_QOL_data(env)        
        # Instantiate the Base class
        self.base = Base(self.driver, extra)         
